...
{   
        ...
        if (in_array($fileActualExt,$allowed)) 
        {
            if ($fileError === 0) {
                if ($fileSize < 5000000) {
                    ...
                    $fileDestination = $_SERVER['DOCUMENT_ROOT'].'/miaa/assets/images/pds/'.$location.'/'.$fileNameNew;
    ...
    {
        unlink($_SERVER['DOCUMENT_ROOT'].'/miaa/assets/images/pds/'.$location.'/'.imgURL($image));
    }

    public function download($app_number,$img,$location)
    {   
        $upload_dir = $_SERVER['DOCUMENT_ROOT'].'/miaa/assets/images/pds/'.$location.'/';
